...
void dataDummy(Logistik logistik[], int &jumlahLogistik) {
    logistik[0] = {"LOG001", {"Batu Bara", "Bituminous", 1200.0, "ton"}, {"Kalimantan"}, "10-02-2025", "Delivered"};
    jumlahLogistik = 1;
}
...
string generateIdRekursif(Logistik logistik[], int jumlahLogistik, int n = 1) {
    ...
    for (int i = 0; i < jumlahLogistik; i++) {
        if (logistik[i].idLogistik == idBaru) {
...
void create(Logistik logistik[], int &jumlahLogistik) {
    ...
    if (jumlahLogistik < MAX_LOGISTIK) {
        ...
        item.idLogistik = generateIdRekursif(logistik, jumlahLogistik);
        ...
        switch (statusPilihan) {
            case 1: item.status = "Pending"; break;
            case 2: item.status = "Shipped"; break;
            case 3: item.status = "Delivered"; break;
        }
...
void update(Logistik logistik[], int jumlahLogistik) {
    ...
    if (index > 0 && index <= jumlahLogistik) {
        ...
        int statusPilihan = inputMenu(1, 3, "Pilih Status: ");
        switch (statusPilihan) {
            case 1: logistik[index - 1].status = "Pending"; break;
            case 2: logistik[index - 1].status = "Shipped"; break;
            case 3: logistik[index - 1].status = "Delivered"; break;
        }
        cout << RESET;
